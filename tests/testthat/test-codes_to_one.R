test_that("Fonctions génériques pour le changement des codes géographiques", {
  ...
  # Même dimension si extra est un vecteur de scission
  y <- codes_to_one(x, codes_ini = ci, codes_fin = cf, extra = ce )
  ...
  dy <- codes_to_one(dx, from = "X", to = "Y",
  ...
  dy <- codes_to_one(dx, from = "X", to = "X",
                     codes_ini = ci, codes_fin = cf, extra = ce)
  ...
  # Résultat attendu avec la fonction codes_to_one()
  y1 <- c("b","c","c","a1","z1")

  # warning si la fonction extra renvoie un vecteur de taille différente
  expect_warning(
    y <- codes_to_one(
      data = x,
      codes_ini = c_ini,
